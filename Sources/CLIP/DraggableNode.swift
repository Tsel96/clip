...
struct DraggableNode: View {
    ...
    private var liveGate: Bool {
        // Dragged cards render their cheap poster so the drag stays smooth.
        if state.draggingNodeIDs.contains(node.id) { return false }
        // Otherwise: visible = live (see `isLive` — no zoom/size LOD pausing).
    ...
    private var nodeContent: some View {
        ...
        case .tweet(let url):
            TweetCardView(
                ...
                onResolveVideoURL: { state.trimVideoURLs[node.id] = $0 },
    ...
    private var chromeCornerRadius: CGFloat {
        ...
        case .tweet, .instagram, .youtube, .webclip, .image, .video, .drawing, .folder:
            return 1     // cards are square (1px) per Figma 88:329
        }
    }
