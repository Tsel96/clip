    ...
    override func mouseDragged(with event: NSEvent) {
        ...
        case .pendingMove, .move:
            if mode == .pendingMove {
                ...
                // Option-drag = duplicate: clone in place and drag the COPIES (the
                // originals stay put — standard app behaviour). The copies render as
                // cheap POSTERS while dragging (state.draggingNodeIDs), so even a
                // heavy web card duplicates smoothly; they go live on release.
