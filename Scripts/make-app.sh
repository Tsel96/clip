...
# (NSBundle.module not found → ClipApp.applyIcon crash). The accessor checks
# `Bundle.main.resourceURL` (= Contents/Resources) FIRST, so it must live there;
# also drop a copy next to the executable for the CLI-style lookup path.
if [ -d "$ROOT/.build/$CONFIG/CLIP_CLIP.bundle" ]; then
  cp -R "$ROOT/.build/$CONFIG/CLIP_CLIP.bundle" "$APP/Contents/Resources/"
  cp -R "$ROOT/.build/$CONFIG/CLIP_CLIP.bundle" "$APP/Contents/MacOS/"
fi
